...
class CallSession:
    ...
    async def start(self, sdp_offer: str) -> Optional[str]:
        ...
        # Greet the caller so they don't have to speak first. Fire-and-forget:
        # the SDP answer must not wait for the LLM round-trip behind this.
        # Tracked on self so hangup() can cancel a still-running greeting.
        if not self._greeting_sent:
            self._greeting_task = asyncio.ensure_future(self._send_greeting())

        logger.info("call %s accepted in room %s", self.call_id[:8], self.room_id)
        return self._pc.localDescription.sdp

    async def _run_preanswer_warmup(self) -> None:
        """Warm STT before answering so the first user utterance isn't dropped.

        Greeting generation is intentionally NOT awaited here — it goes through
        a remote LLM that can take 25s+ and would delay the SDP answer past
        the caller's ringing timeout. The greeting is kicked off as a
        background task in ``start()`` after the answer is queued.
        """
        if not self.PREANSWER_WARMUP_ENABLED:
            return

        started = time.monotonic()
        task = asyncio.create_task(self._warm_stt_with_silence())
        try:
